...
def check_urls_in_text(text: str) -> Tuple[bool, Optional[str], Optional[str]]:
    """
...
def check_no_nsfw_content(text: str) -> Tuple[bool, Optional[str]]:
    """
...
def check_email_addresses(text: str) -> Tuple[bool, Optional[str], Optional[str]]:
    """
...
def ensure_json_parsable(text: str) -> Tuple[bool, Optional[str], Optional[Dict]]:
    """
...
def ensure_required_json_fields(json_obj: Dict, schema_key: str) -> Tuple[bool, Optional[str], Optional[Dict]]:
    """
...
def ensure_no_empty_json_values(json_obj: Dict) -> Tuple[bool, Optional[str], Optional[Dict]]:
    """
...
def run_llm_input_checks(text: str) -> Tuple[bool, List[str], str]:
    """
    ...
    error_messages = []
    fixed_text = text
    length_valid, length_error = check_input_length(text)
    if not length_valid:
        is_valid = False
        error_messages.append(length_error)
        if len(text) > MAX_INPUT_LENGTH:
            fixed_text = text[:MAX_INPUT_LENGTH - 100] + "... [truncated]"
    urls_valid, urls_error, urls_fixed = check_urls_in_text(fixed_text)
    if not urls_valid:
        is_valid = False
        error_messages.append(urls_error)
    elif urls_fixed:
        fixed_text = urls_fixed
    nsfw_valid, nsfw_error = check_no_nsfw_content(fixed_text)
    if not nsfw_valid:
        is_valid = False
        error_messages.append(nsfw_error)
    email_valid, email_error, email_fixed = check_email_addresses(fixed_text)
    if not email_valid:
        is_valid = False
...
def run_llm_output_checks(text: str, expected_format: str = None) -> Tuple[bool, List[str], Any]:
    """
    ...
    if expected_format in ["json", "tool_call", "perception", "plan"]:
        json_valid, json_error, parsed_json = ensure_json_parsable(text)
        if not json_valid:
            is_valid = False
            ...
            if expected_format in JSON_REQUIRED_FIELDS:
                fields_valid, fields_error, fields_fixed = ensure_required_json_fields(parsed_json, expected_format)
                if not fields_valid:
                    is_valid = False
                    error_messages.append(fields_error)
                    parsed_json = fields_fixed
            empty_valid, empty_error, empty_fixed = ensure_no_empty_json_values(parsed_json)
            if not empty_valid:
                is_valid = False
...
def process_llm_call_with_heuristics(input_text: str, output_text: str, expected_format: str = None) -> Dict[str, Any]:
    """
    Apply all input and output heuristics to an LLM call.
    """
    input_valid, input_errors, fixed_input = run_llm_input_checks(input_text)
    output_valid, output_errors, fixed_output = run_llm_output_checks(output_text, expected_format)
    return {
        "input": {
            "original": input_text,
            "valid": input_valid,
...
async def async_validate_and_fix_llm(prompt: str, response: str, expected_format: str = None) -> Tuple[str, str, Dict]:
    """
    Asynchronous function to validate and fix LLM inputs and outputs.
    """
    validation_results = process_llm_call_with_heuristics(prompt, response, expected_format)
